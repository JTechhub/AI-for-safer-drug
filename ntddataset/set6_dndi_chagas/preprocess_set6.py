...
# read data file
df = pd.read_csv(datefile,delimiter='\t')
...
df = df[[     'smiles',      'active ingredient','salt','STATUS', 'VALUE',      'TARGET_NAME',    'protein target','treatment concentration','UNIT','exposure time (h)',  'MEASUREMENT_TYPE', 'type','model type','ASSAY_DESCRIPTION', 'dataset','ASSAY_TYPE','ASSAY_STRAIN','TARGET_TYPE','DOSE','timepoint_unit','SUPPLIER','ASSAY_SOP']]
df.columns = ['smiles code', 'active ingredient','salt','relation','assay value','target disease', 'protein target','treatment concentration','treatment unit','exposure time (h)','measurement','type','model type','description' ,'dataset','ASSAY_TYPE','ASSAY_STRAIN','TARGET_TYPE','DOSE','timepoint_unit','SUPPLIER','ASSAY_SOP']

df.to_csv('set6_result.csv', index=False)
