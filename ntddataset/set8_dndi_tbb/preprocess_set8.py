...
# ASSAY_ID	REFERENCE_ID	ASSAY_DESCRIPTION	ASSAY_TYPE	ASSAY_TEST_TYPE	ASSAY_ORGANISM	ASSAY_STRAIN	ASSAY_TAX_ID	ASSAY_TISSUE	ASSAY_CELL_LINE	ASSAY_SUBCELLULAR_FRACTION	
# TARGET_TYPE	TARGET_NAME	TARGET_ACCESSION	TARGET_ORGANISM	TARGET_TAX_ID	SRC_ID	ASSAY_SOURCE
...
df = df[[     'smiles',      'active ingredient','salt','STATUS', 'VALUE',      'TARGET_NAME',    'protein target','treatment concentration','UNIT','exposure time (h)',  'MEASUREMENT_TYPE', 'ASSAY_TEST_TYPE','model type','ASSAY_DESCRIPTION', 'dataset','REFERENCE_ID','ASSAY_TYPE','ASSAY_ORGANISM','ASSAY_STRAIN','TARGET_TYPE','SRC_ID','ASSAY_SOURCE']]
df.columns = ['smiles code', 'active ingredient','salt','relation','assay value','target disease', 'protein target','treatment concentration','treatment unit','exposure time (h)','measurement','type','model type','description' ,'dataset','REFERENCE_ID','ASSAY_TYPE','ASSAY_ORGANISM','ASSAY_STRAIN','TARGET_TYPE','SRC_ID','ASSAY_SOURCE']

df.to_csv('set8_result.csv', index=False)
